...
public class SINAPIProcessing extends PApplet {

	float pdfWidth = 210.0f;
	float pdfHeight = 148.0f;
	...
	public void setup() {
		...
		String[] lines = loadStrings("mrph_input.txt");
...
//			System.out.println();
	...
	private void drawCategories() {
		...
		for (MorphCategory morphCategory : categories) {
			...
			for (int i = 0; i < morphCategory.getValues().length; i++) {
				...
				text(label, titleCellWidth+i*cellWidth + cellWidth/2.0f - textWidth(label)/2.0f, indexY*cellHeight+cellHeight/2.0f);
			}
			indexY++;
		}
	}
	...
	static public void main(String[] passedArgs) {
		String[] appletArgs = new String[] { "examplePOI.SINAPIProcessing" };
		if (passedArgs != null) {
